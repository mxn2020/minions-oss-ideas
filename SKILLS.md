...
description: Agent skills for working with Minions Oss-ideas MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-oss-ideas data.
---

# Minions Oss-ideas Agent Skills

Skills for agents operating on the `minions-oss-ideas` toolbox.
...
```bash
# TypeScript
pnpm add @minions-oss-ideas/sdk
...
The `oss-ideas` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-oss-ideas/sdk/schemas';
...
```python
from minions_oss_ideas.schemas import custom_types
...
- This agent only writes to `minions-oss-ideas` — it reads from other toolboxes but never writes to them
